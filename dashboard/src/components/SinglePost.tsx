...
import { PostInterface, deletePost } from '../api/postApi';
...
export const SinglePost: React.FC<SinglePostProps> = ({ post }) => {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(['posts', accessToken]),
  });
  const handleOnDelete = () => {
    const confirm = window.confirm(
      'Voulez vous vraiment supprimer ce poste? ' + post._id
    );
    if (!confirm) return;
    mutate({ token: accessToken, id: post._id });
  };
    ...
    <div className="grid grid-cols-6 border-t border-stroke px-4 py-4.5 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
      ...
      <div className="col-span-1 flex items-center gap-8">
        <button
          ...
          className="inline-flex w-fit items-center justify-center rounded-md bg-primary px-0 py-1 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10"
        >
          Signalements {post.reports.length}
        </button>
        <button className="  hover:text-primary" onClick={handleOnDelete}>
          <svg
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden outline-none focus:outline-none">
            <div className="relative mx-auto my-6 w-auto max-w-3xl">
              <div className="relative flex w-full flex-col rounded-lg border-0 bg-white shadow-lg outline-none focus:outline-none">
                <div className="flex flex-col items-center justify-between gap-4 rounded-t  p-5">
                  <h3 className="w-full text-center text-xl font-semibold">
                    Signalements
                  </h3>
                  <div>
                    {post.reports.map((rep) => (
                      <p key={rep._id}>{rep.reason}</p>
                    ))}
                  </div>
                </div>
